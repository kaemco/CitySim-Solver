...
bool cPerezSkyModel::SetSkyConditions(double Idh, double Ibn, double sunAltitude, double sunAzimuth, unsigned int day)
{
    // day 1-365
    day = (day-1)%365+1; // stay within boundaries, JK - 21.06.2015
    ...
    m_SolarAlt = sunAltitude; // in radians
    m_SolarAz  = sunAzimuth;  // in radians

	// store solar zenith
    SolarZenith = M_PI/2. - m_SolarAlt;

    // sets the solar altitude to zero if inferior to 6 degrees (including negative numbers)
	if((m_SolarAlt/M_PI)*180. < 6.)
        m_SolarAlt = 0.;
///ici cette boucle ne fait pas de sens, solor Alt > 0
//	if (m_SolarAlt > 0)
...
//		Ibn=0;
	...
	// calculate clearness
        double pow3= SolarZenith*SolarZenith*SolarZenith;
	PerezClearness = ((Idh+Ibn)/Idh + 1.041*pow3)
                /(1+1.041*pow3);

	//@author Christophe Giller
	if (PerezClearness < 1 && PerezClearness+ 0.000000000000101 >=1.0) PerezClearness = 1;
    //-------------------------
	// calculate brightness
	// extra terrestrial radiation

	// TODO: check IextraT and AirMass eqns with task 3 microclimate modelling paper
	day_angle=day*2*M_PI/365;
...
//	if (m_SolarAlt*180/M_PI + 3.885 >=0)
	...
	else
	{
		...
		#ifdef VERBOSE_CLIMAT_WARNING
		fprintf(stderr,"Error!  Solar altitude < 6 degrees and Idh > 10 W/m^2 (%f) on day %d!  Attempting to continue!\n", Idh, day);
		#endif
		PerezBrightness=0;
	}
